...
const dropdownVariants = {
  ...
  itemText: cva('w-full rounded-md cursor-pointer h-10 md:h-14 text-sm md:font-base flex items-center justify-center'),

  itemInner: cva('rounded-md transition-colors duration-200 flex items-center', {
    variants: {
      state: {
...
function useDropdown({
  ...
  selectedDate,
  onDateChange,
}: Required<Pick<DropdownProps, 'selected' | 'onSelect' | 'placeholder' | 'iconType' | 'selectedDate' | 'onDateChange'>>) {
...
export default function Dropdown({
  ...
  const IconComponent = (() => {
    if (iconType === 'sort') return sortOrder === 'desc' ? SortUpIcon : SortDownIcon;
  ...
  const renderRegularDropdown = () => (
    <ul className={dropdownVariants.menu({ iconType })}>
      {showPlaceholderInMenu && (
        ...
        >
          <div className={dropdownVariants.itemInner({
  ...
  return (
    <div ref={dropdownRef} className={cn(dropdownVariants.container(), className)}>
      <button
        className={cn(dropdownVariants.buttonBase({ iconType }), textColor)}
        onClick={toggleDropdown}
        type="button"
      >
        {iconType === 'sort' && IconComponent && <IconComponent className="w-6 h-6" />}
        {iconType === 'comment' ? (
          <div className="text-sm text-white">⋮</div>
        ) : (
          <span className={dropdownVariants.text({ iconType })}>{displayText}</span>
        )}
        {(iconType === 'arrow' || iconType === 'date') && IconComponent && (
          <IconComponent className={cn('w-6 h-6 ml-2', textColor)} />
        )}
      </button>
      {isOpen && (iconType === 'date' ? renderDatePicker() : renderRegularDropdown())}
    </div>
  );
}
