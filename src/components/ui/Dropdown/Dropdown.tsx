...
export default function Dropdown({
  ...
  return (
    <div
      ref={dropdownRef}
      className={dropdownVariants.container({ type })}
    >
      <button
        className={dropdownVariants.button({ type, active: Boolean(selected) && selected !== placeholder })}
      ...
      >
        {type === '정렬' ? (
          <>
            ...
            />
            <p className={`${dropdownVariants.text({ type })} hidden md:block`}>
              {selected || placeholder}
            </p>
          </>
        ) : (
          <>
            <p className={dropdownVariants.text({ type, active: Boolean(selected) && selected !== placeholder })}>
              {selected || placeholder}
            </p>
            <Image
              src={iconSrc}
